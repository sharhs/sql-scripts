...
N'CREATE UNIQUE '+ CASE WHEN si.index_id > 1 THEN N'NON' ELSE N'' END + N'CLUSTERED ' +' INDEX ' + QUOTENAME(si.name) +
' ON ' + QUOTENAME(sc.name) + N'.' + QUOTENAME(t.name)
...
ALLOW_PAGE_LOCKS = ' +CASE WHEN si.allow_row_locks = 1 THEN ' ON' ELSE + ' OFF' END +
'
)' +
...
WHERE
sc.name = 'core'
AND t.name NOT IN('claim'
,'promocode_itemset_name'
,'promocode_campaign_description'
,'promocode_bonus'
,'promocode_contactset_name')
AND fg.name <> 'CORE'
ORDER BY reserved_in_row_GB DESC, table_name, si.index_id
OPTION (RECOMPILE);
GO
